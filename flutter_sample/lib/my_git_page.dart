...
class MyGitPage extends StatelessWidget {
  const MyGitPage({super.key});
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: GridView.builder(
      ...
      itemBuilder: (context, index) => ClipRRect(
        ...
        child: buildgridTitle(context, index),
      ),
    ),
  );

  Widget buildgridTitle(BuildContext ctx, int index){
    var rnd = Random();
    ...
    return GridTile(
      footer: GridTileBar(
        ...
        trailing: InkWell(
          ...
          onTap: () => myAlert(ctx,'คุณเปิดดูสินค้า ลำดับที่ - $index'),          
        ),
      ),
